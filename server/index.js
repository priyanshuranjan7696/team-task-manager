...
app.get('/api/projects/:projectId', requireAuth, requireProjectRole(['Admin', 'Member']), (req, res) => {
  const project = db
    .prepare(
      'SELECT p.*, pm.role FROM projects p JOIN project_members pm ON pm.project_id = p.id WHERE p.id = ? AND pm.user_id = ?'
    )
...
app.listen(port, '0.0.0.0', () => {
  console.log(`ProjectFlow running on 0.0.0.0:${port}`);
